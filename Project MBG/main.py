"""
main.py – MBG Stock Management API v2.0
...
class InputPrediksi(BaseModel):
    ...
    bulan_ord   : int   = Field(..., ge=9,  le=100,
                                description="Ordinal bulan: 9=Jun2026, 10=Jul2026 dst (1-8=data training Okt25-Mei26)")
...
class JadwalProduksi(BaseModel):
    ...
    sesi                 : str  = Field(..., description="'siang'")
    nama_menu            : str
...
def _seed_inventory_dari_dataset_helper(db: dict):
    """
    Isi inventory awal dari Rekap Nota May 2026 Minggu ke-4.
    Hanya menambahkan bahan yang belum ada, tidak menimpa yang sudah ada.
    """
    import glob

    DATASET_PATH = "Rekap Nota May 2026.xlsx"
    SHEET_W4     = "Rekap 25 - 30 Mei 2026"

    if not os.path.exists(DATASET_PATH):
        raise HTTPException(status_code=404, detail="File Rekap Nota May 2026.xlsx tidak ditemukan.")

    try:
        df = pd.read_excel(DATASET_PATH, sheet_name=SHEET_W4)
        df.columns = ["NO", "ITEM", "VOL", "SAT"]
        df = df.dropna(subset=["ITEM"])
        df["VOL"] = pd.to_numeric(df["VOL"], errors="coerce").fillna(0)
    
        nama_existing = {b["nama"].lower() for b in db["inventory"]}

        for _, row in df.iterrows():
            nama  = str(row["ITEM"]).strip()
            vol   = float(row["VOL"])
            satuan = str(row["SAT"]).strip() if pd.notna(row["SAT"]) else "unit"
    
            if nama.lower() in nama_existing:
                continue
            if nama.lower() in _BLACKLIST_INVENTORY:
                continue
            record = {
                "id"           : _next_id(db["inventory"]),
                "nama"         : nama,
                "satuan"       : satuan,
                "stok_saat_ini": vol,
                "stok_minimum" : round(vol * 0.2, 2),   # 20% dari vol sebagai batas minimum
                "stok_maksimum": round(vol * 5.0, 2),   # 5x vol sebagai batas maksimum
                "kategori"     : "Umum",
                "harga_per_unit": 0.0,
                "supplier"     : "",
                "keterangan"   : "Seeded dari Rekap Nota May 2026 W4",
            }
            db["inventory"].append(record)
            nama_existing.add(nama.lower())
...
def auto_seed_saat_startup():
    """Auto-seed inventory saat server pertama kali jalan, jika inventory kosong."""
    db = _load_db()
    if len(db["inventory"]) == 0:
        _seed_inventory_dari_dataset_helper(db)

@app.post("/inventory/seed-dari-dataset", tags=["Inventory"])
def seed_inventory_dari_dataset():
    """Isi inventory dari Rekap Nota May 2026 W4. Hanya menambah yang belum ada."""
    db = _load_db()
    sebelum = len(db["inventory"])
    _seed_inventory_dari_dataset_helper(db)
    db_after = _load_db()
    sesudah  = len(db_after["inventory"])
    ditambahkan = sesudah - sebelum
...
_BLACKLIST_INVENTORY = {
    nama.lower() for nama in [
        "Gas LPG 3 Kg", "Gas LPG 12 Kg", "Token Listrik", "Air Isi Ulang (Galon)",
        "Es Batu", "HP", "Sabun Cuci Piring (Sunlight/Mama Lemon)", "Sabun Cuci Tangan",
        "Karbol Wangi / Pembersih Lantai", "Spons Cuci Piring", "Kain Lap / Microfiber",
        "Plastik Sampah Besar (Trash Bag)", "Plastik Sampah Kecil", "Tisu Meja",
        "Tisu Wajah", "Paper Box Bento / Kotak Makan", "Sendok Plastik", "Garpu Plastik",
        "Kantong Plastik / Kresek Besar", "Kantong Plastik / Kresek Sedang",
        "Karet Gelang", "Nota Penjualan", "Buku Kas", "Pulpen",
        "Isolatip / Lakban Bening", "Isolatip Fragile",
        "Biaya Transportasi / Bensin Armada", "Biaya Kurir", "Parkir Belanja",
        "Kebersihan Lingkungan", "Biaya Maintenance / Servis Ringan Alat Dapur"
    ]
}
@app.post("/inventory", tags=["Inventory"], status_code=201)
def tambah_bahan(bahan: BahanMakanan):
    if bahan.nama.lower() in _BLACKLIST_INVENTORY:
        raise HTTPException(
            status_code=400,
            detail=f"'{bahan.nama}' bukan bahan makanan dan tidak dapat ditambahkan ke inventory."
        )
    db = _load_db()
...
def tandai_selesai(jadwal_id: int, kurangi_stok: bool = True):
    ...
    # Cek kecukupan stok semua bahan sebelum proses
    bahan_kurang = []
    for kb in jadwal.get("kebutuhan_bahan", []):
        bahan = next((b for b in db["inventory"]
                      if b["nama"].lower() == kb["nama_bahan"].lower()), None)
        stok_tersedia = bahan["stok_saat_ini"] if bahan else 0
        if stok_tersedia < kb["jumlah_dibutuhkan"]:
            bahan_kurang.append(
                f"{kb['nama_bahan']}: butuh {kb['jumlah_dibutuhkan']} "
                f"{kb['satuan']}, tersedia {stok_tersedia}"
            )

    # Jika ada bahan yang kurang, tolak dan jangan kurangi stok
    if bahan_kurang:
        raise HTTPException(
            status_code=400,
            detail={
                "pesan": "Jadwal tidak dapat diselesaikan karena stok bahan tidak mencukupi.",
                "bahan_kurang": bahan_kurang,
            }
        )
    
    # Semua stok cukup, proses pengurangan
    log_transaksi = []
    if kurangi_stok:
        for kb in jadwal.get("kebutuhan_bahan", []):
            bahan = next((b for b in db["inventory"]
